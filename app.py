...
def close_shop():
    ...
    aadhaar = data.get("aadhaar_totals", {})
    aps = aadhaar.get("APS", 0)
    cash = aadhaar.get("CASH", 0)
    gpay = aadhaar.get("GPAY", 0)
    ...
    # Optional SMS report (only if Twilio configured)
    if to_number and TWILIO_SID and TWILIO_TOKEN and TWILIO_FROM:
        try:
            ...
            print("[SMS] Close-shop report sent")
        except Exception as e:
            print("[SMS ERROR]", e)

    return jsonify({"status": "ok", "message": "close report saved"})
